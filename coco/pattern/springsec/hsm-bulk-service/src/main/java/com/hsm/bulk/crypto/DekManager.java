...
/**
 * Duplicated verbatim from com.hsm.core.crypto.DekManager (hsm-core-service) -- this
 * repo has no shared library module between Spring Boot modules (see
 * cek-rotation-service's own FipsBootstrap duplication for precedent), so
 * hsm-bulk-service carries its own copy of this stateless, Spring-free class.
 * Token format MUST stay byte-for-byte identical: a ciphertext token produced
 * here (CLNT-side, via /dek/issue) has to decrypt through hsm-core-service's
 * unmodified /decrypt endpoint (java/docs/BULK_OPERATIONS.md's Tier 3 hard
 * requirement).
 */
