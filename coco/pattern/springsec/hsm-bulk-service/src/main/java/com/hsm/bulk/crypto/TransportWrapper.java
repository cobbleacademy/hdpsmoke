 ...
 * no new crypto primitive is introduced by this PoC.
 *
 * <p>hsm-bulk-service itself only ever calls {@link #wrap}: the app's private key
 * never leaves CLNT by design, so this service can encrypt DEK bytes for a specific
 * app's public key but can never decrypt one back. {@link #unwrap} exists so the
