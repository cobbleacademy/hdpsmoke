...
/** Duplicated from com.hsm.core.auth.RsaJwtValidator -- identical RS256/JWKS validation, only the config type changed (HsmBulkProperties.Jwt, same shape as HsmProperties.Jwt). Points at the SAME JWT_JWKS_URL/audience/issuer as hsm-core-service. */
public class RsaJwtValidator implements JwtValidator {

    private static final long JWKS_TTL_SECONDS = 3600;

    private final HsmBulkProperties.Jwt config;
    ...
    public RsaJwtValidator(HsmBulkProperties.Jwt config) {
        this.config = config;
