 ...
 * Tier 3 PoC -- NOT a JUnit assertion test, a runnable measurement tool. Compares two
 * API shapes on the SAME hsm-core-service instance (dek/issue and encrypt/batch were
 * merged from formerly-separate hsm-bulk-service/hsm-core-service codebases):
 *
 * <p><b>Batch (Tier 1)</b>: plaintext travels to hsm-core-service, which does the
 * AES-GCM encrypt itself, via POST /encrypt/batch.
 *
 * <p><b>Bulk (Tier 3)</b>: hsm-core-service only issues a transport-wrapped DEK via
 ...
 *
 * <h2>Prerequisites (demo-mode-equivalent, no real Azure needed)</h2>
 * <pre>
 *   # terminal 1 -- hsm-core-service, demo mode
 ...
 *
 *   # terminal 2
 *   mvn -pl hsm-core-service test-compile exec:java -Dexec.mainClass=com.hsm.core.bench.BulkVsBatchBenchmark -Dexec.classpathScope=test
 * </pre>
 *
...
public final class BulkVsBatchBenchmark {
    ...
    private static final String SERVICE_URL = System.getProperty("serviceUrl", "http://localhost:3005");
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("records=" + RECORD_COUNT + " chunkSize=" + CHUNK_SIZE + " service=" + SERVICE_URL);
        ...
        System.out.println("\nNote: this runs against MockKekClient locally (no real Azure Managed HSM call),");
    ...
    private static void verifyTokenFormatCompatibility(PrivateKey privateKey) throws Exception {
        ...
        JsonNode issueResponse = postJson(SERVICE_URL + API_PREFIX + "/dek/issue", issueBody);
        ...
        JsonNode decryptResponse = postJson(SERVICE_URL + API_PREFIX + "/decrypt", decryptBody);
    ...
    private static long runBatchPath(List<String> plaintexts) throws Exception {
        ...
        for (int offset = 0; offset < plaintexts.size(); offset += CHUNK_SIZE) {
            ...
            postJson(SERVICE_URL + API_PREFIX + "/encrypt/batch", body);
        }
    ...
    private static long runBulkPath(List<String> plaintexts, PrivateKey privateKey) throws Exception {
        ...
        for (int offset = 0; offset < plaintexts.size(); offset += CHUNK_SIZE) {
            ...
            JsonNode response = postJson(SERVICE_URL + API_PREFIX + "/dek/issue", body);
